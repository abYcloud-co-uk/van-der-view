...
type: decision
status: stable
sources: [raw/0003-design-decisions-2026-06-18.md, raw/0004-testing-strategy-decisions-2026-06-18.md, raw/0007-node-structure-spike-2026-06-18.md, raw/0008-plan2-executor-core-2026-06-18.md, raw/0009-plan3a-browser-runtime-core-2026-06-22.md, raw/0011-plan3b-demo-merged-verified-2026-06-22.md, raw/0012-trajectory-cluster-merged-2026-06-23.md, raw/0013-packaging-merged-2026-06-23.md, "docs/superpowers/specs/2026-06-18-testing-strategy-design.md"]
updated: 2026-06-23
links: [agent-command-flow, command-schema, molstar-api, headless-react, molstar-webxr, molstar-trajectories, packaging]
---
...
### Packaging release gate (PR #19, src: raw/0013)

Packaging added a **release-only gate** separate from the normal Vitest run:
`pnpm verify:package` = `typecheck → test → build → publint → attw → assert-agent-side-molstar-free
→ smoke-dist`. Two new **off-GPU automated checks** ride here (run against built `dist/`, not TS
source, so not in the Vitest suite): `scripts/assert-agent-side-molstar-free.mjs` statically walks
`dist/index.js`'s import graph and fails if any chunk imports molstar (the molstar-free invariant as
an executable gate), and `scripts/smoke-dist.mjs` imports `dist/index.js` in Node and asserts the
agent surface (`commands`/`tools`/`adapters`). `prepublishOnly` runs the gate, so CI and a local
`pnpm publish` both block on it. `pnpm test`/`pnpm typecheck` themselves are unchanged — still on TS
source ([[packaging]]).
...
- [[packaging]] — the `verify:package` release gate + the molstar-free guard
